...
//
/* --------------- YOUR ANSWER HERE ---------------
O(n) because we are going through the for loop once
for num times. Inside the for loop all operations are constant
time
...
/* --------------- YOUR ANSWER HERE ---------------
O(n)
the inner loop always runs 5 times, so that's constant. All we
 have to consider is the outer for loop which runs arr.length times
...
//
/* --------------- YOUR ANSWER HERE ---------------
O(nxm) where n is the row size and m is the col size because
we are going through each element of matrix once which means 
row x col elements in total
...
//
/* --------------- YOUR ANSWER HERE ---------------
for (var j = 2; (j * j) <= i; j++) runs sqrt(i) times;
so sqrt(1)+sqrt(2)+sqrt(3)+sqrt(4)...sqrt(n) = 2/3*n*sqrt(5/4+n)
This gives out complexity of O(n*sqrt(n)). Last for loop's time
will be ignored in front of this.

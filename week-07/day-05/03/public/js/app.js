$(document).ready(function () {
  ...
  $.get("api/restaurants/", function(data){
    for(var i =0;i<data.length;i++)
      renderCards(data[i].restaurant_name, data[i].restaurant_link, data[i].restaurnt_img, data[i].restaurant_id, data[i].rating);
  });
}
getAllRestaurants();
  ...
  $(document).on("click", ".star-click", function(){
    restaurantID = $(this).parent().attr("id");
    var currentURL = window.location.origin;
    $.ajax({
      url: currentURL+"/api/restaurants/"+restaurantID+"/rating/" + $(this).attr("value"),
      type: "PUT"
    }).then(function(){});
  })
...
$("#newRestaurant").on("submit", function(){
  console.log(parseInt($("#rating").text().trim()))
  $.post("/api/restaurants/new", 
  {restaurant_name : $("#restauarant_name").val(),
  restaurant_link : $("#restauarant_link").val(),
  restaurant_img: $("#restauarant_img").val(),
  rating: parseInt($("#rating").val().trim())
  },
  function(data){});
});

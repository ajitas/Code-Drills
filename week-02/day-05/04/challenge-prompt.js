...
console.log(animal.indexOf("says")>-1);


// using `indexOf`, console.log `true` if the string "zebra" is in the `animal` 
// variable and `false` if it's not.
console.log(animal.indexOf("zebra")>-1);
...
var position = animal.lastIndexOf('o');
animal = animal.substring(0,position) + 'z' + animal.substring(position+1);
...
for(var i =0 ,j =letters.length-1; i<letters.length/2; j--,i++){
    var temp = letters[i];
    letters[i] = letters[j];
    letters[j] = temp;
}
...
for(var i = 0;i<letters.length;i++){
    strLetters = strLetters + letters[i];
}
...
function doubleLength(strInput){
...
function arrayMerge(arrayInput){
    var strOutput ="";
    for(var i=0;i<arrayInput.length;i++){
        strOutput = strOutput + arrayInput[i];
    }
    return strOutput;
}
